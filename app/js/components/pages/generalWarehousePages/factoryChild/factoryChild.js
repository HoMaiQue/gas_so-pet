...
class GeneralStaff extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      listUsers: [],
  ...
  async getListFixer() {
    ...
    //   Constants.GENERALWAREHOUSE,
    //   "",
    //   Constants.SUPERADMIN
    // );
    let user_cookies = await getUserCookies();
    let id = user_cookies.user.id;
    const dataUsers = await getListGeneralWarehouse(id);
    if (dataUsers) {
      if (dataUsers.data.success) {
        ...
        <div className="card">
          <div className="card-title">
            <div className="flexbox">
              <h4> {this.state.user_type === "General"?this.props.t('GENERAL_CHILD'):this.props.t('FACTORY_CHILD')} </h4>
              <div className="row">
                <button
                ...
                >
                  {this.state.user_type === "General"?this.props.t('CREATE_EXPORT_GOV'):this.props.t('CREATE_EXPORT_SOPET')}
                </button>
...
export default withNamespaces()(GeneralStaff);
