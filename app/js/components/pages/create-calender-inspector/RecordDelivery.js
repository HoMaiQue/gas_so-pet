...
import "./RecordDelivery.scss";
import moment from 'moment';

class RecordDelivery extends React.Component{
render(){
    // console.log("data",this.props.inspectorGetChecklist,"listInformation",this.props.listInformation)
    let {inspectorGetChecklist,listInformation} =this.props;
    return(
        <div class="modal fade bd-example-modal-lg" id="record-delivery">
        <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title" id="exampleModalLabel"></h5>
            <button type="button" class="close" data-dismiss="modal" aria-label="Close" style={{marginBottom:5}}>
                ...
                <table className="table table-bordered" key={index}>
                    ...
                    <tbody>
                        <tr>
                            ...
                            <th scope="row">Van an toàn:<span className="result">{
                                moment(data.inspectionDate_pipingSystem, moment.ISO_8601).isValid() ? moment(data.inspectionDate_pipingSystem).format('DD/MM/YYYY') : data.inspectionDate_pipingSystem
                            }</span></th>
                        </tr>
                        <tr>
                            <th scope="row">Đồng hồ áp:<span className="result">{
                                moment(data.inspectionDate_safetyValve, moment.ISO_8601).isValid() ? moment(data.inspectionDate_safetyValve).format('DD/MM/YYYY') : data.inspectionDate_safetyValve
                            }</span></th>
                        </tr>
                        <tr>
                            <th scope="row">Đường ống:<span className="result">{
                                moment(data.inspectionDate_pressureGauges, moment.ISO_8601).isValid() ? moment(data.inspectionDate_pressureGauges).format('DD/MM/YYYY') : data.inspectionDate_pressureGauges
                            }</span></th>
                        </tr>
                    </tbody>
                </table>
